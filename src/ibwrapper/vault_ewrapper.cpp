...
#include "ibwrapper/vault_scanner_subscription.h"
#include "ibwrapper/vault_utils.h"
#include "ibwrapper/vault_fa_methods.h"
#include "ibwrapper/vault_available_algo_params.h"
#include "ibwrapper/vault_account_summary_tags.h"
...
bool VaultEWrapper::connect(const char *host, int port, int clientId)
{
    // trying to connect
...
void VaultEWrapper::disconnect() const
{
...
bool VaultEWrapper::isConnected() const
{
    return m_pClient->isConnected();
}

void VaultEWrapper::setConnectOptions(const std::string& connectOptions)
{
    m_pClient->setConnectOptions(connectOptions);
}

void VaultEWrapper::setOptionalCapabilities(const std::string& optionalCapabilities)
{
    m_pClient->setOptionalCapabilities(optionalCapabilities);
}

void VaultEWrapper::processMessages()
{
...
// methods
//! [connectack]
void VaultEWrapper::connectAck() {
    if (!m_extraAuth && m_pClient->asyncEConnect())
        m_pClient->startApi();
}
//! [connectack]

void VaultEWrapper::reqCurrentTime()
{
...
void VaultEWrapper::pnlOperation()
{
    //! [reqpnl]
...
void VaultEWrapper::pnlSingleOperation()
{
    //! [reqpnlsingle]
...
void VaultEWrapper::tickDataOperation()
{
    //! [reqmarketdatatype]
...
void VaultEWrapper::tickOptionComputationOperation()
{
    /*** Requesting real time market data ***/
    std::this_thread::sleep_for(std::chrono::seconds(1));
...
void VaultEWrapper::delayedTickDataOperation()
{
...
void VaultEWrapper::marketDepthOperations()
{
...
void VaultEWrapper::realTimeBars()
{
...
void VaultEWrapper::marketDataType()
{
    //! [reqmarketdatatype]
...
void VaultEWrapper::historicalDataRequests()
{
...
void VaultEWrapper::optionsOperations()
{
    //! [reqsecdefoptparams]
...
void VaultEWrapper::contractOperations()
{
...
void VaultEWrapper::marketScanners()
{
...
void VaultEWrapper::fundamentals()
{
...
void VaultEWrapper::accountOperations()
{
...
void VaultEWrapper::orderOperations()
{
...
void VaultEWrapper::conditionSamples()
{
...
void VaultEWrapper::bracketSample(){
    Order parent;
    Order takeProfit;
    Order stopLoss;
    //! [bracketsubmit]
...
void VaultEWrapper::hedgeSample(){
    //F Hedge order
    //! [hedgesubmit]
...
void VaultEWrapper::testAlgoSamples(){
    //! [algo_base_order]
...
void VaultEWrapper::financialAdvisorOrderSamples()
{
    //! [faorderoneaccount]
...
void VaultEWrapper::financialAdvisorOperations()
{
...
void VaultEWrapper::testDisplayGroups(){
    //! [querydisplaygroups]
...
void VaultEWrapper::miscelaneous()
{
...
void VaultEWrapper::reqFamilyCodes()
{
...
void VaultEWrapper::reqMatchingSymbols()
{
...
void VaultEWrapper::reqMktDepthExchanges()
{
...
void VaultEWrapper::reqNewsTicks()
{
    //! [reqmktdata_ticknews]
...
void VaultEWrapper::reqSmartComponents()
{
...
void VaultEWrapper::reqNewsProviders()
{
...
void VaultEWrapper::reqNewsArticle()
{
...
void VaultEWrapper::reqHistoricalNews(){
...
void VaultEWrapper::reqHeadTimestamp() {
    //! [reqHeadTimeStamp]
...
void VaultEWrapper::reqHistogramData() {
    //! [reqHistogramData]
...
void VaultEWrapper::rerouteCFDOperations()
{
    //! [reqmktdatacfd]
...
void VaultEWrapper::marketRuleOperations()
{
...
void VaultEWrapper::continuousFuturesOperations()
{
...
void VaultEWrapper::reqHistoricalTicks()
{
    //! [reqhistoricalticks]
...
void VaultEWrapper::reqTickByTickData()
{
...
void VaultEWrapper::whatIfSamples()
{
...
void VaultEWrapper::ibkratsSample(){
    //! [ibkratssubmit]
...
void VaultEWrapper::wshCalendarOperations() {
    //! [reqmetadata]
...
void VaultEWrapper::rfqOperations()
{
    //! [rfq_submission]
...
//! [nextvalidid]
void VaultEWrapper::nextValidId( OrderId orderId)
{
...
void VaultEWrapper::currentTime( long time)
{
...
//! [error]
void VaultEWrapper::error(int id, int errorCode, const std::string& errorString, const std::string& advancedOrderRejectJson)
{
...
//! [tickprice]
void VaultEWrapper::tickPrice( TickerId tickerId, TickType field, double price, const TickAttrib& attribs) {
...
//! [ticksize]
void VaultEWrapper::tickSize( TickerId tickerId, TickType field, Decimal size) {
    printf( "Tick Size. Ticker Id: %ld, Field: %d, Size: %s\n", tickerId, (int)field, DecimalFunctions::decimalStringToDisplay(size).c_str());
}
//! [ticksize]

//! [tickoptioncomputation]
void VaultEWrapper::tickOptionComputation( TickerId tickerId, TickType tickType, int tickAttrib, double impliedVol, double delta,
...
//! [tickgeneric]
void VaultEWrapper::tickGeneric(TickerId tickerId, TickType tickType, double value) {
    printf( "Tick Generic. Ticker Id: %ld, Type: %d, Value: %s\n", tickerId, (int)tickType, Utils::doubleMaxString(value).c_str());
}
//! [tickgeneric]

//! [tickstring]
void VaultEWrapper::tickString(TickerId tickerId, TickType tickType, const std::string& value) {
    printf( "Tick String. Ticker Id: %ld, Type: %d, Value: %s\n", tickerId, (int)tickType, value.c_str());
}
//! [tickstring]

void VaultEWrapper::tickEFP(TickerId tickerId, TickType tickType, double basisPoints, const std::string& formattedBasisPoints,
...
//! [orderstatus]
void VaultEWrapper::orderStatus(OrderId orderId, const std::string& status, Decimal filled,
...
//! [openorder]
void VaultEWrapper::openOrder( OrderId orderId, const Contract& contract, const Order& order, const OrderState& orderState) {
...
//! [openorderend]
void VaultEWrapper::openOrderEnd() {
    printf( "OpenOrderEnd\n");
}
//! [openorderend]

void VaultEWrapper::winError( const std::string& str, int lastError) {}
void VaultEWrapper::connectionClosed() {
    printf( "Connection Closed\n");
}

//! [updateaccountvalue]
void VaultEWrapper::updateAccountValue(const std::string& key, const std::string& val,
...
//! [updateportfolio]
void VaultEWrapper::updatePortfolio(const Contract& contract, Decimal position,
...
//! [updateaccounttime]
void VaultEWrapper::updateAccountTime(const std::string& timeStamp) {
    printf( "UpdateAccountTime. Time: %s\n", timeStamp.c_str());
}
//! [updateaccounttime]

//! [accountdownloadend]
void VaultEWrapper::accountDownloadEnd(const std::string& accountName) {
    printf( "Account download finished: %s\n", accountName.c_str());
}
//! [accountdownloadend]

//! [contractdetails]
void VaultEWrapper::contractDetails( int reqId, const ContractDetails& contractDetails) {
...
//! [bondcontractdetails]
void VaultEWrapper::bondContractDetails( int reqId, const ContractDetails& contractDetails) {
...
void VaultEWrapper::printContractMsg(const Contract& contract) {
    printf("\tConId: %ld\n", contract.conId);
...
void VaultEWrapper::printContractDetailsMsg(const ContractDetails& contractDetails) {
...
void VaultEWrapper::printContractDetailsSecIdList(const TagValueListSPtr &secIdList) {
...
void VaultEWrapper::printContractDetailsIneligibilityReasonList(const IneligibilityReasonListSPtr &ineligibilityReasonList) {
...
void VaultEWrapper::printBondContractDetailsMsg(const ContractDetails& contractDetails) {
...
//! [contractdetailsend]
void VaultEWrapper::contractDetailsEnd( int reqId) {
    printf( "ContractDetailsEnd. %d\n", reqId);
}
//! [contractdetailsend]

//! [execdetails]
void VaultEWrapper::execDetails( int reqId, const Contract& contract, const Execution& execution) {
...
//! [execdetailsend]
void VaultEWrapper::execDetailsEnd( int reqId) {
    printf( "ExecDetailsEnd. %d\n", reqId);
}
//! [execdetailsend]

//! [updatemktdepth]
void VaultEWrapper::updateMktDepth(TickerId id, int position, int operation, int side,
...
//! [updatemktdepthl2]
void VaultEWrapper::updateMktDepthL2(TickerId id, int position, const std::string& marketMaker, int operation,
...
//! [updatenewsbulletin]
void VaultEWrapper::updateNewsBulletin(int msgId, int msgType, const std::string& newsMessage, const std::string& originExch) {
    printf( "News Bulletins. %d - Type: %d, Message: %s, Exchange of Origin: %s\n", msgId, msgType, newsMessage.c_str(), originExch.c_str());
}
//! [updatenewsbulletin]

//! [managedaccounts]
void VaultEWrapper::managedAccounts( const std::string& accountsList) {
    printf( "Account List: %s\n", accountsList.c_str());
}
//! [managedaccounts]

//! [receivefa]
void VaultEWrapper::receiveFA(faDataType pFaDataType, const std::string& cxml) {
    std::cout << "Receiving FA: " << (int)pFaDataType << std::endl << cxml << std::endl;
}
//! [receivefa]

//! [historicaldata]
void VaultEWrapper::historicalData(TickerId reqId, const Bar& bar) {
...
//! [historicaldataend]
void VaultEWrapper::historicalDataEnd(int reqId, const std::string& startDateStr, const std::string& endDateStr) {
    std::cout << "HistoricalDataEnd. ReqId: " << reqId << " - Start Date: " << startDateStr << ", End Date: " << endDateStr << std::endl;
}
//! [historicaldataend]

//! [scannerparameters]
void VaultEWrapper::scannerParameters(const std::string& xml) {
    printf( "ScannerParameters. %s\n", xml.c_str());
}
//! [scannerparameters]

//! [scannerdata]
void VaultEWrapper::scannerData(int reqId, int rank, const ContractDetails& contractDetails,
...
//! [scannerdataend]
void VaultEWrapper::scannerDataEnd(int reqId) {
    printf( "ScannerDataEnd. %d\n", reqId);
}
//! [scannerdataend]

//! [realtimebar]
void VaultEWrapper::realtimeBar(TickerId reqId, long time, double open, double high, double low, double close,
...
//! [fundamentaldata]
void VaultEWrapper::fundamentalData(TickerId reqId, const std::string& data) {
    printf( "FundamentalData. ReqId: %ld, %s\n", reqId, data.c_str());
}
//! [fundamentaldata]

void VaultEWrapper::deltaNeutralValidation(int reqId, const DeltaNeutralContract& deltaNeutralContract) {
...
//! [ticksnapshotend]
void VaultEWrapper::tickSnapshotEnd(int reqId) {
    printf( "TickSnapshotEnd: %d\n", reqId);
}
//! [ticksnapshotend]

//! [marketdatatype]
void VaultEWrapper::marketDataType(TickerId reqId, int marketDataType) {
    printf( "MarketDataType. ReqId: %ld, Type: %d\n", reqId, marketDataType);
}
//! [marketdatatype]

//! [commissionreport]
void VaultEWrapper::commissionReport( const CommissionReport& commissionReport) {
...
//! [position]
void VaultEWrapper::position( const std::string& account, const Contract& contract, Decimal position, double avgCost) {
...
//! [positionend]
void VaultEWrapper::positionEnd() {
    printf( "PositionEnd\n");
}
//! [positionend]

//! [accountsummary]
void VaultEWrapper::accountSummary( int reqId, const std::string& account, const std::string& tag, const std::string& value, const std::string& currency) {
    printf( "Acct Summary. ReqId: %d, Account: %s, Tag: %s, Value: %s, Currency: %s\n", reqId, account.c_str(), tag.c_str(), value.c_str(), currency.c_str());
}
//! [accountsummary]

//! [accountsummaryend]
void VaultEWrapper::accountSummaryEnd( int reqId) {
    printf( "AccountSummaryEnd. Req Id: %d\n", reqId);
}
//! [accountsummaryend]

void VaultEWrapper::verifyMessageAPI( const std::string& apiData) {
    printf("verifyMessageAPI: %s\b", apiData.c_str());
}

void VaultEWrapper::verifyCompleted( bool isSuccessful, const std::string& errorText) {
    printf("verifyCompleted. IsSuccessfule: %d - Error: %s\n", isSuccessful, errorText.c_str());
}

void VaultEWrapper::verifyAndAuthMessageAPI( const std::string& apiDatai, const std::string& xyzChallenge) {
    printf("verifyAndAuthMessageAPI: %s %s\n", apiDatai.c_str(), xyzChallenge.c_str());
}

void VaultEWrapper::verifyAndAuthCompleted( bool isSuccessful, const std::string& errorText) {
...
//! [displaygrouplist]
void VaultEWrapper::displayGroupList( int reqId, const std::string& groups) {
    printf("Display Group List. ReqId: %d, Groups: %s\n", reqId, groups.c_str());
}
//! [displaygrouplist]

//! [displaygroupupdated]
void VaultEWrapper::displayGroupUpdated( int reqId, const std::string& contractInfo) {
    std::cout << "Display Group Updated. ReqId: " << reqId << ", Contract Info: " << contractInfo << std::endl;
}
//! [displaygroupupdated]

//! [positionmulti]
void VaultEWrapper::positionMulti( int reqId, const std::string& account,const std::string& modelCode, const Contract& contract, Decimal pos, double avgCost) {
...
//! [positionmultiend]
void VaultEWrapper::positionMultiEnd( int reqId) {
    printf("Position Multi End. Request: %d\n", reqId);
}
//! [positionmultiend]

//! [accountupdatemulti]
void VaultEWrapper::accountUpdateMulti( int reqId, const std::string& account, const std::string& modelCode, const std::string& key, const std::string& value, const std::string& currency) {
...
void VaultEWrapper::accountUpdateMultiEnd( int reqId) {
    printf("Account Update Multi End. Request: %d\n", reqId);
}
//! [accountupdatemultiend]

//! [securityDefinitionOptionParameter]
void VaultEWrapper::securityDefinitionOptionalParameter(int reqId, const std::string& exchange, int underlyingConId, const std::string& tradingClass,
...
void VaultEWrapper::securityDefinitionOptionalParameterEnd(int reqId) {
    printf("Security Definition Optional Parameter End. Request: %d\n", reqId);
}
//! [securityDefinitionOptionParameterEnd]

//! [softDollarTiers]
void VaultEWrapper::softDollarTiers(int reqId, const std::vector<SoftDollarTier> &tiers) {
...
//! [familyCodes]
void VaultEWrapper::familyCodes(const std::vector<FamilyCode> &familyCodes) {
...
//! [symbolSamples]
void VaultEWrapper::symbolSamples(int reqId, const std::vector<ContractDescription> &contractDescriptions) {
...
//! [mktDepthExchanges]
void VaultEWrapper::mktDepthExchanges(const std::vector<DepthMktDataDescription> &depthMktDataDescriptions) {
...
//! [tickNews]
void VaultEWrapper::tickNews(int tickerId, time_t timeStamp, const std::string& providerCode, const std::string& articleId, const std::string& headline, const std::string& extraData) {
...
//! [smartcomponents]]
void VaultEWrapper::smartComponents(int reqId, const SmartComponentsMap& theMap) {
...
//! [tickReqParams]
void VaultEWrapper::tickReqParams(int tickerId, double minTick, const std::string& bboExchange, int snapshotPermissions) {
...
//! [newsProviders]
void VaultEWrapper::newsProviders(const std::vector<NewsProvider> &newsProviders) {
...
//! [newsArticle]
void VaultEWrapper::newsArticle(int requestId, int articleType, const std::string& articleText) {
...
//! [historicalNews]
void VaultEWrapper::historicalNews(int requestId, const std::string& time, const std::string& providerCode, const std::string& articleId, const std::string& headline) {
...
//! [historicalNewsEnd]
void VaultEWrapper::historicalNewsEnd(int requestId, bool hasMore) {
    printf("Historical News End. RequestId: %d, HasMore: %s\n", requestId, (hasMore ? "true" : " false"));
}
//! [historicalNewsEnd]

//! [headTimestamp]
void VaultEWrapper::headTimestamp(int reqId, const std::string& headTimestamp) {
...
//! [histogramData]
void VaultEWrapper::histogramData(int reqId, const HistogramDataVector& data) {
...
void VaultEWrapper::historicalDataUpdate(TickerId reqId, const Bar& bar) {
...
//! [rerouteMktDataReq]
void VaultEWrapper::rerouteMktDataReq(int reqId, int conid, const std::string& exchange) {
    printf( "Re-route market data request. ReqId: %d, ConId: %d, Exchange: %s\n", reqId, conid, exchange.c_str());
}
//! [rerouteMktDataReq]

//! [rerouteMktDepthReq]
void VaultEWrapper::rerouteMktDepthReq(int reqId, int conid, const std::string& exchange) {
    printf( "Re-route market depth request. ReqId: %d, ConId: %d, Exchange: %s\n", reqId, conid, exchange.c_str());
}
//! [rerouteMktDepthReq]

//! [marketRule]
void VaultEWrapper::marketRule(int marketRuleId, const std::vector<PriceIncrement> &priceIncrements) {
...
//! [pnl]
void VaultEWrapper::pnl(int reqId, double dailyPnL, double unrealizedPnL, double realizedPnL) {
...
//! [pnlsingle]
void VaultEWrapper::pnlSingle(int reqId, Decimal pos, double dailyPnL, double unrealizedPnL, double realizedPnL, double value) {
...
//! [historicalticks]
void VaultEWrapper::historicalTicks(int reqId, const std::vector<HistoricalTick>& ticks, bool done) {
...
void VaultEWrapper::historicalTicksBidAsk(int reqId, const std::vector<HistoricalTickBidAsk>& ticks, bool done) {
...
//! [historicaltickslast]
void VaultEWrapper::historicalTicksLast(int reqId, const std::vector<HistoricalTickLast>& ticks, bool done) {
...
//! [tickbytickalllast]
void VaultEWrapper::tickByTickAllLast(int reqId, int tickType, time_t time, double price, Decimal size, const TickAttribLast& tickAttribLast, const std::string& exchange, const std::string& specialConditions) {
    char timeStr[80];
#if defined(IB_WIN32)
...
//! [tickbytickbidask]
void VaultEWrapper::tickByTickBidAsk(int reqId, time_t time, double bidPrice, double askPrice, Decimal bidSize, Decimal askSize, const TickAttribBidAsk& tickAttribBidAsk) {
    char timeStr[80];
#if defined(IB_WIN32)
    ctime_s(timeStr, sizeof(timeStr), &time);
#else
...
//! [tickbytickmidpoint]
void VaultEWrapper::tickByTickMidPoint(int reqId, time_t time, double midPoint) {
    char timeStr[80];
#if defined(IB_WIN32)
    ctime_s(timeStr, sizeof(timeStr), &time);
#else
...
//! [orderbound]
void VaultEWrapper::orderBound(long long orderId, int apiClientId, int apiOrderId) {
...
//! [completedorder]
void VaultEWrapper::completedOrder(const Contract& contract, const Order& order, const OrderState& orderState) {
...
//! [completedordersend]
void VaultEWrapper::completedOrdersEnd() {
    printf( "CompletedOrdersEnd\n");
}
//! [completedordersend]

//! [replacefaend]
void VaultEWrapper::replaceFAEnd(int reqId, const std::string& text) {
    printf("Replace FA End. Request: %d, Text:%s\n", reqId, text.c_str());
}
//! [replacefaend]

//! [wshMetaData]
void VaultEWrapper::wshMetaData(int reqId, const std::string& dataJson) {
    printf("WSH Meta Data. ReqId: %d, dataJson: %s\n", reqId, dataJson.c_str());
}
//! [wshMetaData]

//! [wshEventData]
void VaultEWrapper::wshEventData(int reqId, const std::string& dataJson) {
    printf("WSH Event Data. ReqId: %d, dataJson: %s\n", reqId, dataJson.c_str());
}
//! [wshEventData]

//! [historicalSchedule]
void VaultEWrapper::historicalSchedule(int reqId, const std::string& startDateTime, const std::string& endDateTime, const std::string& timeZone, const std::vector<HistoricalSession>& sessions) {
...
//! [userInfo]
void VaultEWrapper::userInfo(int reqId, const std::string& whiteBrandingId) {
